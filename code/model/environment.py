...
class Episode(object):
    ...
    def __init__(self, graph, data, params):
        ...
        self.current_hop = 0
        start_entities, query_relation,  end_entities, all_answers = data  #e1, r, e2, all_e2s
        ...
        next_actions,weight_ret = self.grapher.return_next_actions(self.current_entities, self.start_entities, self.query_relation,
                                                        self.end_entities, self.all_answers, self.current_hop == self.path_len - 1,
                                                        self.num_rollouts)
        self.state = {}
        self.state['next_relations'] = next_actions[:, :, 1]
        self.state['next_relations_weight'] = weight_ret
    ...
    def get_reward(self):
        ...
        reward = np.select(condlist, choicelist)  # [B,]  #choose one of the two value of choicelist
        return reward
...
class env(object):
    ...
    def get_episodes(self):
        ...
        if self.mode == 'train':
            for data in self.batcher.yield_next_batch_train():  #e1, r, e2, all_e2s
